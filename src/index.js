...
const API_KEY = 'live_LdyZZAkvOUOZW3p4VB6sUES1xXuWB4URiEdzXmtox4uhzgcwlKHnYfmIPsy30pkJ';
...
fetchBreeds(BASE_URL, API_KEY)
  .then(data => {
    ...
    const addBreedCats = data
      .map(breedCat => `<option value='${breedCat.id}'>${breedCat.name}</option>`)
      .join('');
...
.then(data => {
  ...
  const addCatInfo = data
    .map(infoCat => `${infoCat.name}, ${infoCat.description}, ${infoCat.temperament}`)
    .join('');
